...
** test JIT idea
Make a tiny synth that just expects me to hit a key,
and whenever I do it makes a voice, and release when I release it.
** Vivid: voice creation, use and deletion
use "synth" to make a synth, "free" to free it:
  :set -XDataKinds
  import Vivid as V
  s <- synth boop ()
  V.set s (toI 0.5 :: I "amp")
  free s
...
* JI synth
** show played notes
print the grid, with a star next to each held note,
or at least the most recently pressed note.
* infrastructure
** add tests
