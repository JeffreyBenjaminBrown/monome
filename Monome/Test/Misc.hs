...
testBelongsHere = TestCase $ do
  let w1 = Window "w1" (\(x,y) -> x > y) id $ (\_ st -> Right st)
      w2 = Window "w2" (\(x,_) -> x > 4) id $ (\_ st -> Right st)
      w3 = Window "w3" (\(_,_) -> True)  id $ (\_ st -> Right st)
      ws = [w1,w2,w3]
